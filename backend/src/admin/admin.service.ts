...
export class AdminService {
  ...
  async login(username: string, password: string) {
    try {
      ...
      // Superadmin por variables de entorno (DESHABILITADO si no se configura)
      // IMPORTANTE: No hardcodear credenciales en el código.
      const SUPER_ADMIN_USERNAME = process.env.SUPER_ADMIN_USERNAME;
      const SUPER_ADMIN_PASSWORD = process.env.SUPER_ADMIN_PASSWORD;
      const SUPER_ADMIN_NAME = process.env.SUPER_ADMIN_NAME || 'Super Administrador';

      const isEnvSuperAdminEnabled = !!SUPER_ADMIN_USERNAME && !!SUPER_ADMIN_PASSWORD;
      const isEnvSuperAdminLogin =
        isEnvSuperAdminEnabled &&
        username.toLowerCase() === SUPER_ADMIN_USERNAME!.toLowerCase() &&
        password === SUPER_ADMIN_PASSWORD;

      // Verificar si es el superadmin configurado por env
      if (isEnvSuperAdminLogin) {
        this.logger.log('🔐 Login con superadmin (env)');
